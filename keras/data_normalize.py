...
def get_data(filename):
    ...
    # save_obj(vector,r"C:\Users\Master\PycharmProjects\untitled1\data")
...
def get_random_sequence(data, char_map, id_map, avg, length = 50):
    ...
    if(isinstance(data[rnd_key][rnd_index], float)):
        rnd_index += 1;
...
if __name__ == '__main__':
    ...
    for i in range (0,10000):
        x.append(get_random_sequence(data, char_map, id_map, avg, 50))
        x.append(get_random_drunk_sequence(data, char_map, id_map, avg, 50))
        y.append([1])
        y.append([0])
    print(x)
    ...
    model = tf.Sequential()
    model.add(tf.layers.Conv1D(strides = 2, kernel_size=2, filters=120, input_shape=(1,100)))
    model.add(tf.layers.Activation('relu'))
    model.add(tf.layers.Conv1D(kernel_size=5, filters=10))
    model.add(tf.layers.Flatten())
    model.add(tf.layers.Dropout(0.4))
    model.add(tf.layers.Dense(2, activation='relu'))
    model.add(tf.layers.Dense(1, activation='relu'))

    model.compile(loss='binary_crossentropy', optimizer=tf.optimizers.SGD(lr=5))
    model.summary()

    model.fit(X, Y, batch_size=1000, epochs=10000)

    allow_map = {v: k for k, v in id_map.items()}
